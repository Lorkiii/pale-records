...
export function ClassScheduleFields({
  ...
  const selectedWeekdays = new Set(
    rows.map((row) => row.dayOfWeek).filter(Boolean),
  );
  ...
  return (
    <fieldset
      className="border-t border-paper-border pt-5"
      aria-describedby="class-schedule-help">
      <legend className="font-display text-lg font-semibold tracking-[-0.02em] text-ink">
        Weekly schedule
      </legend>
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <p
          id="class-schedule-help"
          className="mt-1 max-w-xl text-sm leading-6 text-ink-muted">
          Add up to one time range per weekday. Times cannot overlap another
          active class on the same day.
        </p>
        <Button
          type="button"
          variant="secondary"
          onClick={addRow}
          disabled={disabled || rows.length >= 7}>
          Add schedule day
        </Button>
      </div>
      ...
      {sectionError ? (
        <Notice
          variant="error"
          title="Weekly schedule needs attention"
          className="mt-4">
          {sectionError}
        </Notice>
      ) : null}
        ...
        <div className="mt-4 space-y-3">
          {rows.map((row) => {
            ...
            return (
              <div
                key={row.key}
                className="grid gap-3 border border-paper-border bg-paper-light p-3 sm:grid-cols-[minmax(0,1fr)_minmax(8rem,0.7fr)_minmax(8rem,0.7fr)_auto] sm:items-end">
                <Select
                  ...
                  value={row.dayOfWeek}
                  onChange={(event) =>
                    updateRow(row.key, "dayOfWeek", event.target.value)
                  }
                  disabled={disabled}
                  ...
                  options={[
                    ...
                    ...CLASS_WEEKDAYS.map((weekday) => ({
                      ...
                      label: weekday.label,
                      disabled:
                        row.dayOfWeek !== String(weekday.value) &&
                        selectedWeekdays.has(String(weekday.value)),
                    })),
                  ]}
                />
                <Input
                ...
                <Input
                  ...
                  onChange={(event) =>
                    updateRow(row.key, "endTime", event.target.value)
                  }
                  disabled={disabled}
